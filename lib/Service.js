...
class Service {
  ...
  add(name, method) {
    ...
    if (lodash.isString(name) && lodash.isPlainObject(method)) {
      method = this.types.Method.fromObject(name, method);
    }
    else if (name instanceof this.types.Method && method === undefined) {
      method = name;
      name = undefined;
    }

    // Assert the method is a Method.
    assert(method instanceof this.types.Method, 'method must be an instance of Method');
  ...
  set methods(methods) {
    ...
    if (lodash.isArray(methods)) {
      if (~(methods.map((method) => method instanceof this.types.Method).indexOf(false))) {
    ...
    // Convert hash object to Method instances.
      ...
      lodash.forOwn(objects, (object, name) => {
        methods.push(this.types.Method.fromObject(name, object));
      });
    }
